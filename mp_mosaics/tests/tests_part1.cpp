...
TEST_CASE("KDTree::testSmallerDimVal Tests", "[weight=1][part=1]") {
  ...
  REQUIRE( smallerDimVal(a, b, 2) == false );
  REQUIRE( smallerDimVal(a, b, 1) == true );   // based on operator<
}
...
TEST_CASE("select simple","[weight=1][part=1]") {
  vector<int>numbers = {5, 42, 6, 21, 1, 99, 57, 87, 27};
...
//   std::string fname = "kdtree_"+to_string(size)+"-actual.kd";

//   ofstream outputFile(fname, ofstream::out);
//   tree.printTree(outputFile, colored_out::DISABLE, -1);
//   outputFile.close();

//   compareBinaryFiles(fname, "../data/kdtree_"+to_string(size)+"-expected.kd" );
// }
